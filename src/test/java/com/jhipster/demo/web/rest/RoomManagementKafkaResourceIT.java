...
import com.jhipster.demo.config.KafkaProperties;
...
class RoomManagementKafkaResourceIT {
    ...
    void setup() {
        ...
        RoomManagementKafkaResource kafkaResource = new RoomManagementKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/room-management-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/room-management-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
